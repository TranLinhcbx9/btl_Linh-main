...
namespace Eshopper.Areas.Admin.Controllers
{
    public class SanPhamsController : BaseController
    {
        ...
        public ActionResult Index(int? startPrice, int? endPrice, int? page, string searchString ="" )
        {
            var items = db.SanPhams.ToList();
            //if(searchString != string.Empty)
            //{
            //    items = items.Where(x => (searchString!=string.Empty || (x.TenSP.ToLower().Trim().Contains(searchString.ToLower().Trim()))) 
            //    && ((!startPrice.HasValue ||  x.DonGia >= startPrice) && (!endPrice.HasValue || x.DonGia<=endPrice)) ).ToList();
            //}
            items = items.Where(x => ((searchString == string.Empty) || (x.TenSP.ToLower().Trim().Contains(searchString.ToLower().Trim())))
               ).ToList();

            items = items.Where(x => ((!startPrice.HasValue || x.DonGia >= startPrice) && (!endPrice.HasValue || x.DonGia <= endPrice))).ToList();

            var pageNumber = page == null ? 1: page.Value;
            var data = items.OrderByDescending(x=>x.MaSP).ToPagedList(pageNumber, 5);
            return View(data);
        }
        ...
        {
            bool isExist = db.SanPhams.Any(x => x.MaKM.Contains(sanPham.MaSP));
            if (ModelState.IsValid)
            {
                if(isExist)
                {
                    
                    return Content("<script language='javascript' type='text/javascript'>alert('Mã sản phẩm đã tồn tại');</script>");
                }
                else
                {
        ...
        public ActionResult DeleteConfirmed(string id)
        {
            ...
            bool hasNhap = db.CTPhieuNhaps.Any(x => x.MaSP.Contains(sanPham.MaSP));
            bool hasXuat = db.CTPhieuXuats.Any(x => x.MaSP.Contains(sanPham.MaSP));

            if(hasNhap || hasXuat)
            {
                return Content("<script language='javascript' type='text/javascript'>alert('Sản phẩm này đang thuộc phiếu nhập, xuất');</script>");
            }
            else
            {
